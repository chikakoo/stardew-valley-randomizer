...
public class MonsterRandomizer
{
	...
	/// 
	/// Excludes gliders to prevent monsters from getting stuck in the wall
	/// </summary>
	...
	{
		// TODO in 0.8.0: adjust on release - no need for the rng calls if not a glider

		var newThreshold = Rng.NextBoolean(5)
			? Rng.NextIntWithinRange(8, 12)
			: Rng.NextIntWithinRange(0, 4);

		if (!monster.IsGlider)
		{
			monster.MovesTowardPlayerThreshold = newThreshold;
		}
	}
